...
# ``google.generativeai`` import is **deferred** through ``_genai()`` so the
# module can be imported by lightweight callers (e.g. inspecting templates,
# generating offline content via local fallback) without paying the Gemini
# SDK import cost — which also emits a FutureWarning at module load time.


def _genai():
    """Lazy accessor for the ``google.generativeai`` module.

    Returns the imported module; subsequent calls hit ``sys.modules`` so the
    cost is paid exactly once per process and only when something actually
    needs Gemini.
    """
...
class ContentGenerator:
    ...
    def _setup_api(self):
        ...
        try:
            ...
            # Configure the Gemini API with the key (lazy import of SDK)
    ...
    def _generate_local_post(self, topic, default_post=None):
        ...
        final_post = post or (
            default_post or f"Sharing a few thoughts on {topic} today."
        )
    ...
    def generate_post_content(self, topic):
        ...
        default_post = (
            matched_post
            or f"Exploring the fascinating world of {topic} today. Innovation and adaptation are key in this rapidly evolving landscape. I'd love to hear your insights on this topic! #ProfessionalDevelopment #IndustryTrends #LinkedIn"
        )

        # Enhanced logging to show which template is being used
        if matched_post:
            logging.info(
                f"Using matched template for '{matched_key}' keyword in topic: '{topic}'"
            )
        else:
        ...
        try:
            ...
            if hasattr(config, "USE_GEMINI") and not config.USE_GEMINI:
                logging.info(
                    "AI generation disabled (USE_GEMINI=False). Using local fallback content."
                )
            ...
            if selected_model:
                ...
                client = _genai().GenerativeModel(selected_model)
                ...
                if post_response and hasattr(post_response, "text"):
                    ...
                    logging.warning(
                        "Received invalid response from Gemini API, using local fallback content"
                    )
            else:
                logging.warning(
                    "No suitable model found for content generation, using local fallback content"
                )
    ...
    def _select_gemini_model(self):
        ...
        try:
            ...
            preferred_models = ["gemini-2.5-flash", "gemini-1.0-pro", "gemini-pro"]
            selected_model = None
            ...
            for preferred in preferred_models:
                for i, model_name in enumerate(extracted_models):
                    if preferred in model_name:
                        ...
                        logging.info(
                            f"Found matching model: {preferred} -> {selected_model}"
                        )
                        break
                if selected_model:
                    break
            ...
            if not selected_model:
                ...
                for model_info in models:
                    if "generateContent" in model_info.supported_generation_methods:
                        ...
                        logging.info(
                            f"Using alternative text generation model: {selected_model}"
                        )
                        break
            ...
            if not selected_model:
                for model_name in preferred_models:
                    try:
                        ...
                        client = _genai().GenerativeModel(model_name)
    ...
    def _call_gemini_api_with_retries(
        self, client, messages, max_retries=3, base_delay=5
    ):
        ...
        while retry_count < max_retries:
            try:
                ...
                if "429" in str(e) and retry_count < max_retries - 1:
                    ...
                    logging.info(
                        f"Rate limited. Retry {retry_count+1}/{max_retries} in {delay} seconds..."
                    )
                    time.sleep(delay)
                    retry_count += 1
                else:
    ...
    def generate_content_calendar(self, niche: str, total_posts: int = 30) -> list[str]:
        """Generate a list of content ideas for a specific niche.

        Why:
            Populate a topic file with a month's worth of relevant ideas.

        When:
            Called via the `generate-calendar` CLI command.

        Args:
            niche (str): The industry or topic area.
            total_posts (int): Number of ideas to generate.

        Returns:
            list[str]: A list of generated topic strings.
        """
        logging.info(
            f"Generating content calendar for niche: {niche} ({total_posts} posts)"
        )

        if not config.GEMINI_API_KEY:
            logging.error("GEMINI_API_KEY not found. Cannot generate calendar.")
            return []
        ...
        try:
            ...
            prompt = (
                f"Generate a content calendar with {total_posts} entries for the '{niche}' niche, starting from {formatted_date}. "
                "Format each line exactly as: Date | Title | Description | Hook | Hashtags\n"
                "Example: 2023-10-27 | Quick Wins | A 10-minute routine for busy pros. | 'No time? Try this.' | #Fitness #QuickWorkout\n"
                "Ensure dates are consecutive. Provide ONLY the list lines, no header, no numbering."
            )

            messages = [{"role": "user", "parts": [prompt]}]
            response = self._call_gemini_api_with_retries(client, messages)

            if response and response.text:
                # Clean up the response
                lines = response.text.strip().split("\n")
                # Remove numbering/bullets if the AI included them despite instructions
                cleaned_lines = [
                    re.sub(r"^[\d\-\*\.]+\s*", "", line).strip()
                    for line in lines
                    if line.strip()
                ]
                return cleaned_lines[:total_posts]
