...
# ``sumy`` (and its NLTK dependency) is **deferred** to
# :func:`summarize_post_text`. This keeps importing the engage chain cheap and
# — critically — keeps the rest of the bot functional when sumy / NLTK aren't
# installed: in that case ``summarize_post_text`` falls back to whitespace
# normalisation instead of crashing the whole subprocess at import time.
...
def summarize_post_text(text: str, sentences: int = 3) -> Optional[str]:
    ...
    try:
        # Deferred import so the module is usable even when sumy / NLTK aren't
        # installed; in that case we fall through to the plain-text fallback.
        ...
        # First call only — sumy / NLTK aren't installed. Log loudly so the
        # user can install them; downstream still gets readable text.
        try:
            logging.warning(
                "SUMMARIZE fallback: sumy/NLTK not installed (%s). Install with"
                " `pip install sumy nltk` and `python -c \"import nltk;"
                " nltk.download('punkt')\"`.",
                err,
            )
        except Exception:
