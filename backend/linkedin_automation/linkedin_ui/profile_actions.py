        ...
        try:
            ...
            try:
                ...
                for selector in selectors:
                    try:
                        profile_elements = WebDriverWait(self.driver, 5).until(
                            EC.presence_of_all_elements_located((By.XPATH, selector))
                        )
                        if profile_elements:
                            logging.info(
                                f"Found {len(profile_elements)} profiles using selector: {selector}"
                            )
                            break
                    except Exception:
                        continue
                ...
                if not profile_elements:
                    raise Exception("No profile elements found with any known selector")
                ...
                for idx, profile in enumerate(profile_elements, 1):
                    try:
                        ...
                        logging.debug(
                            f"Profile {idx} text sample: {profile_text[:120]}..."
                        )

                        if any(
                            keyword in profile_text for keyword in normalized_keywords
                        ):
                            logging.info(
                ...
                try:
                    ...
                    try:
                        self.driver.execute_script(
                            "arguments[0].click();", show_all_link
                        )
                    except Exception:
                        show_all_link.click()
                    time.sleep(2)
