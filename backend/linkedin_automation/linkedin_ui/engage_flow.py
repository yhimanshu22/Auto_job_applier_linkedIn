...
class EngageExecutor:
    ...
    def _handle_empty_viewport(self) -> bool:
        ...
        How:
            ...
            to aggressive loading when necessary. After a small number of
            consecutive empty viewports the executor aborts — that usually
            means LinkedIn is serving a stripped feed (headless detection /
            checkpoint redirect) and continuing wastes minutes.
        ...
        # First time we hit an empty feed, drop a diagnostic snapshot so we
        # can see *why* (wrong page, stripped DOM, etc.) without rerunning.
        if not self.ctx.diag_dumped:
            self._dump_diagnostics(reason="no_posts_found")
            self.ctx.diag_dumped = True
        ...
        # If we keep landing on empty viewports the feed is almost certainly
        # broken (LinkedIn often serves an empty shell to headless / flagged
        # sessions). Bail with a clear error after a few attempts.
        post_scroll = self.x._find_visible_posts(limit=4)
        if not post_scroll:
            self.ctx.empty_viewports += 1
            if self.ctx.empty_viewports >= 3:
                logging.error(
                    "ENGAGE_ABORT empty_viewports=%d — feed never produced any posts. "
                    "Most common cause: headless detection. Disable headless in "
                    "Dashboard > LinkedIn Automation > Settings (or set HEADLESS=false) "
    ...
    def _dump_diagnostics(self, reason: str) -> None:
        """Save the current page HTML + a screenshot for offline inspection.

        Why:
            Empty-feed failures look identical in logs whether the cause is
            headless detection, a checkpoint redirect, or a selector drift.
            A captured DOM + image lets us tell them apart in seconds.

        When:
            Called once per engage run, on the first empty viewport.

        How:
            Writes to ``logs/diag/<reason>_<timestamp>.{html,png}``. All work
            is best-effort: any individual failure is swallowed so a broken
            ``page_source`` call can't take down the engage run itself.
        """
        import os
        import time as _t
        ...
        try:
            ...
            slug = f"{reason}_{stamp}"

            try:
                url = self.x.driver.current_url
            except Exception:
                url = "<unavailable>"
            try:
                title = self.x.driver.title
            except Exception:
                title = "<unavailable>"
            logging.info(
                "ENGAGE_DIAG reason=%s url=%s title=%r dir=%s",
                reason,
                url,
                title,
                base,
